...
//断言和前置条件是在运行时发生的检查。在执行某些代码之前，我们可以使用它们来确保满足基本条件。如果断言或前置条件中的布尔条件为true，则代码将照常执行。如果为false，则程序的当前状态无效，代码执行结束，应用程序终止运行。
//断言和前置条件之间的区别在于它们的检查时间：仅在Debug版本中检查断言，但在Debug和Release版本中都会检查前置条件。在Release版本中，不会检查断言中的条件。这意味着我们可以在开发过程中使用任意数量的断言，而不会影响Release中的性能。
//另外，一些情况下代码中某些特定条件没有满足的时候，代码将会暂停执行。你可以使用assertion来终止代码的运行，以此来调试某些bug。
...
//我们可以在原型设计和早期开发期间使用fatalError(_:file:line:)函数，通过编写fatalError("Unimplemented")来创建尚未实现的功能的存根。因为fatalError错误永远不会被优化。
//与断言或前置条件不同，我们可以确保代码执行过程总是在遇到存根(fatalError)时停止。
