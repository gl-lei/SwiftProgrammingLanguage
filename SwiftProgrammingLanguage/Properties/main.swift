...
//MARK: - Properties
//类、结构体和枚举类型可以定义属性。存储属性用来存储类例的常量或者变量的值，计算属性用来对存储属性进行操作而不是存储。
//类、结构体和枚举类型都可以定义计算属性
//只有类和结构体能定义存储属性，枚举不可以定义
...
//这种行为的原因是因为结构体是值类型，引用类型就与之不同，引用类型仍然可以改变变量存储属性的值。
...
//标识为lazy的存储属性被多个线程同时访问的时候，不保证只会初始化一次。

//MARK: - Stored Properties and Instance Variables
//在OC中，可以通过属性或者直接访问类中的实例变量。我们也可以指定类的实例变量来作为属性底层存储的变量。、
//Swift中统一将这些结构归结到单条属性声明中。Swift中的属性没有对应的类实例变量，并且属性底层的实例变量无法直接访问。所有的关于属性的信息：名称、类型、内存管理等等都被定义到类型定义中的某个地方中。

//MARK: - Computed Properties
//类、结构体以及枚举都可以定义计算属性，计算属性不会提供存储值，它提供getter方法和一个可选的setter方法来直接返回和设置其他属性和值。
struct Point {
    var x = 0.0, y = 0.0
}
...
//只有getter没有setter的计算属性被称为只读计算属性
//需要注意，计算属性都必须标识为var。
struct Cuboid {
...
//属性观察者观察并且响应属性值的变化。每当属性的值发生变化的时候，属性观察者就会被调用，即使属性的新值与当前的值一样也不例外。
...
//注意，当在子类的初始化方法中设置继承自父类的属性的时候，父类的属性所对应的willSet和didSet观察者会被调用。
class StepCounter {
...
//存储属性和计算属性对于全局变量和局部变量同样适用。全局变量和局部变量全都是存储变量。存储变量与存储属性类型，提供了确定类型值的存储并且允许设置和返回类型值。
//然而，你同样可以在全局或者局部作用域中定义计算变量和为存储变量定义观察者。计算变量用来计算而不是存储一个值，与计算属性的写法一样。
//全局常量和变量都是懒加载，与懒存储属性一样。不一样的地方是，全局变量和常量前面不需要加lazy标志。局部变量和常量永远不能懒加载。
...
//我们可以定义属于类型本身的属性，而不属于类型的任何一个实例。这种属性被称为类型属性。
//存储类型属性可以是变量或者常量，计算类型属性只能是变量。
//需要注意的是，存储类型属性需要初始值。这是因为类型本身是没有初始化函数的，不能给类存储属性赋初始值。存储类型属性总是懒初始化的，他们被确保只加载一次，在多线程环境下也是如此。此外，不需要使用lazy关键字来修饰。
